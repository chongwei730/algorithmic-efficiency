...
def scale_by_adam(
  ...
  """Rescale updates according to the generalized adam algorithm.
...
def instantiate_optimizer(
  ...
  if disable_preconditioning:
    ...
    return optax.chain(
      scale_by_adam(b1=b1, b2=b2, nesterov=nesterov),
      optax.add_decayed_weights(weight_decay),
      scale_by_learning_rate(learning_rate))
...
def init_optimizer_state(workload: spec.Workload,
  ...
  opt_init_fn, opt_update_fn = instantiate_optimizer(
      ...
      disable_preconditioning=hyperparameters.disable_preconditioning,
...
def update_params(workload: spec.Workload,
  ...
  lr = jax_cosine_warmup(workload.step_hint, hyperparameters)(global_step)
