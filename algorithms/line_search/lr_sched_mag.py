...
class LineSearchScheduler():
    ...
    def test_update_restore_max_diff(self, alpha):
        ...
        logging.warning(f"[TEST] alpha={alpha:.3e}, max |param diff| = {max_diff:.3e}")
        return max_diff
    ...
    def step(self, closure, condition="armijo", c1=0.6, factor=0.5, amax=1.0, amin=1e-6, step=0, interval=100, warmup_length=100, log_dir="./", start_lr=1):
        ...
        if k != 0: 
            ...
            logging.warning(
                "denom=%f, line_search_magnitude=%f, prev=%f, magnitude=%f, lr=%f",
                denom, self.line_search_magnitude, self.prev_magnitude, self.magnitude, lr
            )
        ...
        logging.warning("\n========== Line Search Debug ==========")
        logging.warning(f"loss (phi0): {phi0}")
        logging.warning(f"derphi0: {derphi0}")

        logging.warning("\n--- Norms ---")
        logging.warning(f"grad_norm: {grad_norm:.6f}")
        logging.warning(f"dir_norm:  {dir_norm:.6f}")
        logging.warning(f"param_norm:{param_norm:.6f}")

        # logging.warning("\n--- Dot Products ---")
        # logging.warning(f"g·d: {dot_gd:.6f}")
        # logging.warning(f"g·p: {dot_gp:.6f}")
        # logging.warning(f"cos(g,d): {cos_gd:.6f}")

        # logging.warning("\n--- Max Values ---")
        # logging.warning(f"max|grad|: {max_grad:.6f}")
        # logging.warning(f"max|dir|:  {max_dir:.6f}")
        ...
        # logging.warning(f"[LineSearchScheduler] alpha={alpha:.6g}, fc={fc}")
        ...
        logging.warning("LINESEARCH LR:", alpha, "magnitude:", self.line_search_magnitude)
        if step < warmup_length:
...
def search_bisection_visual(phi, phi0, derphi0, c1,
    ...
    armijo_old_work = phi_a <= phi0 + c1 * alpha * derphi0
    logging.warning("find")
    if armijo_old_work:
        logging.warning("work")
        explored.append((alpha, phi_a))
        for _ in range(num_search):
            logging.warning("maybe")
            new_alpha = alpha * grow
            if new_alpha >= amax:
                break
            new_phi = phi(new_alpha)
