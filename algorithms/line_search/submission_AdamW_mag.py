...
from .lr_sched_mag import LineSearchScheduler
import time
...
def init_optimizer_state(
  ...
  scheduler = LineSearchScheduler(optimizer=optimizer, num_search=16, start_lr=0, model_paras=list(model_params.parameters()), optimizer_type="AdamW", injection=False, search_mode="bisection")
...
#   warmup_length = int(0.01 * workload.step_hint)
  if global_step % line_search_interval == 0:
    ...
    def make_closure():
      def closure(require_grad=False, batch=batch_ls):
        ...
        for b in batch:
          ...
          # 1. construct current gradient direction in the optimizer (before backward)
          if require_grad:
            vecs_before = []
            ...
            if len(vecs_before) > 0:
              g_before = torch.cat(vecs_before)
            else:
              g_before = torch.tensor([], device=device)

            # backward to accumulate gradient for this micro-batch
    ...
    logging.warning("LINESEARCH!!!")
    
    magnitude, dir_norm = scheduler.step(
                closure,
                c1=c1,
                step=global_step,
                interval=line_search_interval,
                condition="armijo",
                warmup_length=line_search_interval,
                log_dir=log_dir,
                start_lr=lr
            )
