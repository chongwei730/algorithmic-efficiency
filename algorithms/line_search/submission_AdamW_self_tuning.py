...
def update_params(
  ...
  line_search_interval = int(round(HPARAMS["interval"] * workload.step_hint))
  closure = None
  ...
  if global_step % line_search_interval == 0 or global_step == warmup_length:
    ...
    def make_closure():
      def closure(require_grad=False, batch=batch_ls):
        ...
        assert count == HPARAMS["accum_steps"]
  ...
  c1 = HPARAMS["c1"] + (1 - HPARAMS["c1"]) * (global_step / workload.step_hint)
  scheduler.step(
                closure,
                c1=c1,
                step=global_step,
...
def data_selection(
  ...
  if global_step % line_search_interval != 0 and global_step != warmup_length:
    ...
    n_search_batches = HPARAMS["accum_steps"]
